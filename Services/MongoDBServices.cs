...
namespace CLIMATE_REST_API.Services
{
    ...
    public class MongoDBServices
    {
        ...
        private readonly IMongoCollection<SensorDataModel> 
        _weatherCollection;
        ...
        IOptions<MongoDBSettings> mongodbSettings)
        {
            ...
            IMongoDatabase database = client.GetDatabase(mongodbSettings
            .Value
            .Database);

            _weatherCollection = database.GetCollection<SensorDataModel>(mongodbSettings
            .Value
            .Collection1);

            _userCollection = database.GetCollection<UserModel>(mongodbSettings
            .Value
            .Collection2);
        }
        #endregion
        ...
        public async Task<SensorDataModel> GetSensorByIdAsync(
        string id)
        {
            var id_filter = Builders<SensorDataModel>
            .Filter
            .Eq(u => u.Id, id);
        ...
        {
            var device_filter = Builders<SensorDataModel>
            .Filter
            .Eq(u => u.Device, device);

            var time_filter = Builders<SensorDataModel>
            .Filter
            .Lt(u => u.Time, DateTime
            .Now
            .AddMonths(-5));

            var project_stage = Builders<SensorDataModel>
            .Projection
            .Expression(
            u => new
            {
        ...
        {
            var filter_1 = Builders<SensorDataModel>
            .Filter
            .Gt(u => u.Time, date_time_start);

            var filter_2 = Builders<SensorDataModel>
            .Filter
            .Lt(u => u.Time, date_time_end);

            var filters = Builders<SensorDataModel>
            .Filter
        ...
        {
            FilterDefinition<SensorDataModel> filter = Builders<SensorDataModel>
            .Filter
            .Eq(u => u.Id, id);

            UpdateDefinition<SensorDataModel> update = Builders<SensorDataModel>
            .Update
            .Set(u => u.Precipitation_mm_h, precipitation_mm_h);
        ...
        {
            var api_token_filter = Builders<UserModel>
            .Filter
            .Eq(u => u.ApiToken, api_token);

            var user = await _userCollection.Find(api_token_filter)
            .FirstOrDefaultAsync();
        ...
        public async Task<bool> CreatedUserAsync(UserModel user_model)
        {
            ...
            .Filter
            .Eq(u => u.UserEmail, user_model.UserEmail);

            var existing_user = await _userCollection
            .Find(filter)
            .FirstOrDefaultAsync();
        ...
        {
            var filter_date_time_start = Builders<UserModel>
            .Filter
            .Gte(u => u.LoginDate, date_time_start);

            var filter_date_time_end = Builders<UserModel>
            .Filter
            .Lte(u => u.LoginDate, date_time_end);

            var filters = Builders<UserModel>
            .Filter
            .And(filter_date_time_start, filter_date_time_end);

            var users = _userCollection.Find(filters)
            .FirstOrDefault();
        ...
        public async Task UpdateUserLoginTimeAsync(string api_token, DateTime login_date_time)
        {
            ...
            var update = Builders<UserModel>
            .Update
            .Set(u => u.LoginDate, login_date_time);
        ...
        public async Task<UserModel?> PatchUsersRoleAsync(string property, object value, DateTime date_time_start, DateTime date_time_end)
        {
            var filter_date_time_start = Builders<UserModel>
            .Filter
            .Gte(u => u.CreatedDate, date_time_start);

            var filter_date_time_end = Builders<UserModel>
            .Filter
            .Lte(u => u.CreatedDate, date_time_end);

            var filters = Builders<UserModel>
            .Filter
            .And(filter_date_time_start, filter_date_time_end);

            var value_property_update = Builders<UserModel>
            .Update
            .Set(property, value);
